...
# BatchNorm2d = lambda x: nn.GroupNorm(1, x, eps=1e-6)
BatchNorm2d = lambda x: nn.InstanceNorm2d(x, eps=1e-6)
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, input_nc=1, output_nc=1):
        extra = config
        ...
        self.out_dims = pre_stage_channels

        hidden_dim = 512
        in_dim = int(np.sum(self.out_dims))
        group_nc = math.gcd(in_dim, hidden_dim)
        ...
        self.fuse = [
            nn.Conv2d(in_dim, hidden_dim, 7, 1, 3, groups=group_nc, bias=False),
            nn.InstanceNorm2d(hidden_dim, eps=1e-6),
            nn.ReLU(),
        ...
        for i in range(n_upsample):
            ...
            self.decoder += [
                ...
                nn.InstanceNorm2d(int(ngf * mult / 2), eps=1e-6),
                nn.ReLU(relu_inplace)]
        self.decoder += [nn.Conv2d(ngf, output_nc, kernel_size=3, padding=1, padding_mode="reflect")]
    ...
    def forward(self, x):
        ...
        y_list[1] = self.up2x(y_list[1])
        y_list[2] = self.up4x(y_list[2])
        y_list[3] = self.up8x(y_list[3])

        y_list = torch.cat(y_list, dim=1)
        y_list = self.fuse(y_list)

        return self.decoder(y_list)


def get_hrnet(cfg, input_nc=1, output_nc=1):
    from .config import MODEL_CONFIGS
    model = HighResolutionNet(MODEL_CONFIGS[cfg], input_nc=input_nc, output_nc=output_nc)
    return model
